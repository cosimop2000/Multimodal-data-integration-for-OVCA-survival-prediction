...
def main():
    ...
    for run in range (runs):
        ...
        train_data, test_data, val_data = torch.utils.data.random_split(data, [int(train_size*len(data))+2, int(test_size*len(data)), int(validation_size*len(data))])
        ...
        val_loader = torch.utils.data.DataLoader(dataset=val_data, batch_size=59, shuffle=True)
        ...
        for epoch in range(EPOCH):
            ...
            for iter, valdata in enumerate(val_loader):
                val_view1_inputs, val_view2_inputs, val_labels = valdata
                val_labels = torch.squeeze(val_labels)

                if USE_GPU:
                    val_view1_inputs, val_view2_inputs, val_labels \
                        = Variable(val_view1_inputs.cuda()), \
                          Variable(val_view2_inputs.cuda()), \
                           val_labels.cuda()
                else:
                    val_view1_inputs = Variable(val_view1_inputs).type(torch.FloatTensor)
                    val_view2_inputs = Variable(val_view2_inputs).type(torch.FloatTensor)
                    val_labels = Variable(val_labels).type(torch.LongTensor)

                view1_specific, view1_shared, view2_specific, view2_shared,\
                siamese_code, classification_output = \
                    model.forward(val_view1_inputs, val_view2_inputs)
                # calculation validation acc
                _, predicted = torch.max(classification_output.data, 1)
                acc, pre, auc, recall = measure_performance(classification_output, val_labels)
                val_acc += acc
                val_count += 1
            ...
            run_acc[run] = test_acc_[best_epoch]
            run_pre[run] = test_pre_[best_epoch]
            run_auc[run] = test_auc_[best_epoch]
            run_recall[run] = test_recall_[best_epoch]
    print(f'ACC:{np.mean(run_acc)}, PRE:{np.mean(run_pre)}, AUC:{np.mean(run_auc)}, RECALL:{np.mean(run_recall)}')
    #save as json the results
    with open('results_ttv.json', 'w') as f:
